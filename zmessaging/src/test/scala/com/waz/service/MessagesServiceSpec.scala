...
class MessagesServiceSpec extends AndroidFreeSpec {
  ...
  scenario("Create a quote") {
    ...
    val quote = service.addTextMessage(convId, "aaa", expectsReadReceipt = false).flatMap { msg1 =>
      ...
      service.addReplyMessage(msg1.id, "bbb", expectsReadReceipt = false).collect { case Some(msg2) => (msg2.contentString, msg2.quote.map(_.message)) }
    }
