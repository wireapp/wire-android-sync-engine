...
object ZLog2 {
  ...
  object LogShow {
    ...
    implicit val UploadAssetGeneral: LogShow[UploadAsset[UploadGeneral]] = logShowWithHash
    ...
    implicit val EncryptionShow: LogShow[Encryption] =
      LogShow.createFrom {
        ...
        case AES_CBC_Encryption(_) => l"AES_CBC_Encryption(**HIDDEN**)"
      }
    ...
    implicit val AssetDetailsLogShow: LogShow[AssetDetails] =
      createFrom {
        case BlobDetails => l"BlobDetails"
        case ImageDetails(dims) => l"ImageDetails($dims)"
        case AudioDetails(duration, loudness) => l"AudioDetails($duration, $loudness)"
        case VideoDetails(duration, dims) => l"VideoDetails($duration, $dims)"
      }

    implicit val Loudness: LogShow[Loudness] =
      LogShow.createFrom { l =>
        import l._
        l"Loudness($levels)"
      }

    implicit val LocalSource: LogShow[LocalSource] =
      LogShow.createFrom { s =>
        import s._
        l"LocalSource($uri, $sha)"
      }
    ...
    implicit val AudioEffectShow: LogShow[GlobalRecordAndPlayService.Audio] = logShowWithToString
    ...
    implicit val StateLogShow: LogShow[GlobalRecordAndPlayService.State] =
      LogShow.createFrom {
        ...
        case Recording(_, key, start, mime, entry, _) => l"Recording(key: $key, start: $start, mime: $mime, entry: $entry)"
      }
