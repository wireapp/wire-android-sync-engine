...
import com.koushikdutta.async.future.{Future => KFuture}
...
object ClientWrapper {
  ...
  implicit def koushiToCancellable[T](f: KFuture[T]): CancellableFuture[T] = {
    val p = Promise[T]()

    new Thread(new Runnable {
      override def run(): Unit = p.complete(Try{ f.get })
    }).start
  ...
  def toKoushi(wrapper: ClientWrapper): AsyncHttpClient = wrapper match {
